...
async function ollamaStatus() { const baseUrl = process.env.OLLAMA_URL || "http://127.0.0.1:11434"; try { const response = await fetch(`${baseUrl}/api/tags`, { signal: AbortSignal.timeout(800) }); if (!response.ok) throw new Error(); const { models = [] } = await response.json(); return { enabled: true, reachable: true, model: process.env.OLLAMA_MODEL || models[0]?.name || null }; } catch { return { enabled: true, reachable: false, model: process.env.OLLAMA_MODEL || null }; } }
