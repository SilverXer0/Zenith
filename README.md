...
This updates the task list, counters, daily focus, and completion summary without reloading the page or clearing an unsaved capture draft. The task panel also supports text search across titles, notes, and projects plus a project filter. Planning and Calendar refresh on load, task changes, manual refresh, page visibility, and roughly every five minutes while open. It is not offline editing or closed-app push: browsers may suspend background pages, and the Mac host must remain running and reachable. Memory still has its own separate refresh behavior.
