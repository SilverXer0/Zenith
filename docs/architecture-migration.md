...
3. **Existing core API parity (in progress):** memory, briefing/planning/summary, optional read-only Calendar, and local Ollama routes are implemented. Calendar uses the shared schema, refreshes access, and contributes events without making task planning depend on Google. The assistant is loopback-only, receives owner-scoped context, produces bounded proposals, requires a separate confirmation request, applies confirmed groups atomically, and can unload a running model. Automated Ollama checks use a simulator, so real Qwen/VRAM evidence remains a cutover gate. Next port the optional voice services. Do not substitute fake “available” statuses for unavailable integrations.
