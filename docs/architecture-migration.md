...
| Multiple calendar connections | Current Calendar storage supports one Google connection per Zenith user | Support multiple named Google accounts/calendars with independent connection, enable/disable, refresh, and disconnect behavior; merge events into planning context |
...
6. **Multiple Calendar connections (next):** migrate the single-account Calendar schema and API to support multiple named connections, then add independent UI controls and merged event/planning behavior. Keep each connection account-scoped and read-only.
