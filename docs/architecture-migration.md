...
| Google Calendar read access | Node UI; Node and Python OAuth/read APIs with shared SQLite data; target frontend shows optional status, connect and disconnect controls plus schedule context | Actual Google OAuth on Windows/Tailscale, frontend cutover and schedule/timezone verification |
...
| Speech input and spoken replies | Optional local adapters in the Node prototype and bounded authenticated Python API; target frontend exposes microphone capture and on-demand spoken replies when configured | Real Whisper/Windows speech-engine execution and device microphone/speaker testing |
