...
3. **Existing core API parity (in progress):** memory, briefing/planning/summary and optional read-only Calendar routes are implemented. Calendar uses the shared schema, refreshes access, and contributes events without making task planning depend on Google. Next port Ollama and voice services, with confirmation still required for every model-proposed mutation. Do not substitute fake “available” statuses for unavailable integrations.
