...
- Optional read-only Google Calendar OAuth, multiple named connections, event reads, refreshable sessions, and planning context.
...
The Python API now supports authenticated Calendar status, connection listing, connect, per-connection rename/pause/disconnect, aggregated upcoming-event reads, and a public OAuth callback protected by a random, one-time state that expires after ten minutes. Each successful authorization creates a separate named connection. Authorization requests offline access and only `https://www.googleapis.com/auth/calendar.readonly`. Access tokens are refreshed per connection when close to expiry and once after a Calendar 401 response. A failed or malformed remote response produces a generic local error and never exposes Google response bodies or credentials.
...
Calendar remains optional. If client settings are absent, Core starts normally, status reports `configured: false`, no Google request is attempted, and explicit connect/event requests return 409. `GET /api/calendar/connections` returns connection metadata without tokens; `PATCH /api/calendar/connections/{id}` changes its display name or enabled state; and `DELETE /api/calendar/connections/{id}` removes only that connection. Only enabled connections contribute events. A temporary failure in one connection does not hide successful events from the others; if every enabled connection fails, planning reports Calendar unavailable. OAuth states, access tokens and refresh tokens are account-scoped in the local SQLite database. Existing single-account `calendar_accounts` data is migrated into the new connection table and a compatibility mirror remains for the transitional Node runtime. The database is local but is not encrypted by the Zenith passphrase, so protect the Mac account and database backup like other private files.

For development, set `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`. The callback defaults to `http://127.0.0.1:3000/api/calendar/oauth/callback`; set `GOOGLE_REDIRECT_URI` to the exact authorized HTTPS callback for Tailscale or a frontend proxy. `GOOGLE_TOKEN_URL` and `GOOGLE_CALENDAR_URL` exist only for isolated compatibility tests and should retain their defaults in normal use.
...
`GET /api/assistant/status` is a public readiness check. Authenticated `POST /api/assistant/chat` sends the signed-in owner's tasks, user-managed context notes, available seven-day Calendar context, and the requested local-timezone availability/task-fit projection to one installed Ollama model. Chat history is limited to the latest eight entries, individual inputs and replies are bounded, and context sections have fixed size budgets. The data sections are explicitly marked as untrusted in the system prompt so instructions embedded in a task, note, or Calendar title are not treated as commands.
...
The included `scripts/whisper-transcribe.py` adapter uses `faster-whisper` and can run with CUDA FP16; its model is downloaded/cached by the adapter runtime, and the process exits after each request so its model resources can be released. On the active M1 Mac, `scripts/mlx-whisper-transcribe.py` uses the Apple MLX Whisper package and defaults to `mlx-community/whisper-base-mlx`; it also exits after each request so the model process can release its resources. The included `scripts/pyttsx3-speak.py` adapter uses a local speech engine on Windows. On macOS, `scripts/macos-say-speak.py` uses the built-in `say` and `afconvert` commands for local WAV output without an extra package. Keep Whisper's optional package in a separate voice virtual environment rather than adding it to Core’s runtime requirements. The first MLX/faster-whisper transcription downloads the selected model; later transcription stays local.
...
The read-only planning routes are `GET /api/briefing`, `GET /api/briefing/morning`, `GET /api/weekly-plan`, and `GET /api/summaries/daily`. They preserve the prototype response shapes and deterministic ordering. Daily focus prefers overdue tasks, then today's tasks, dates and priorities, with recent updates breaking ties. Morning includes overdue, due-today and the next three days. Weekly planning returns seven calendar dates plus undated tasks. Completed tasks are excluded from open-task projections. Tasks may include an optional `estimatedMinutes` value from 5 to 480. Morning and weekly routes accept an optional `timezone=IANA/TZ`; the frontend sends the browser timezone so Calendar ranges follow the user's local midnight and daylight-saving rules. `GET /api/planning/availability?date=YYYY-MM-DD&timezone=IANA/TZ` uses the enabled Calendar connections to return local workday windows, merged busy periods, overlapping-event conflicts, and up to five open tasks that fit those windows. Tasks without an estimate use a transparent 60-minute default. Availability and recommendations are deterministic and do not require Ollama.
...
These checks do not prove real Tailscale access, actual Google OAuth/Calendar reads, real Qwen inference or VRAM release, actual Whisper/Mac speech-engine execution, microphone/speaker operation, PWA installation, notification delivery, or full API parity. The next migration gate is real Mac/phone verification of the responsive Next.js/TypeScript/Tailwind app, followed by the final Windows cutover. See [the migration plan](../docs/architecture-migration.md) for the full scope.

The target Next frontend now exists and is covered by its own checks. The remaining migration gate is real Mac/phone and integration verification; Windows cutover follows that gate. See the Mac/phone runbook, Windows cutover guide, and migration plan.
